...
func RedelegateCmdV3() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cobra.MinimumNArgs(3),
		RunE: func(cmd *cobra.Command, args []string) error {
			validatorAddress, err := cli.ResolveAddress(args[0], flags.ChainID, flags.URI)
			if err != nil {
				return err
			}
			formerValidatorAddress, err := cli.ResolveAddress(args[1], flags.ChainID, flags.URI)
			if err != nil {
				return err
			}
...
func WhitelistCandidateCmdV3() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cobra.MinimumNArgs(3),
		RunE: func(cmd *cobra.Command, args []string) error {
			candidateAddress, err := cli.ResolveAddress(args[0], flags.ChainID, flags.URI)
			if err != nil {
				return err
			}
			amount, err := cli.ParseAmount(args[1])
...
func CheckDelegatorRewardsCmdV3() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cobra.MinimumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			address, err := cli.ParseAddress(args[0], &flags)
			if err != nil {
				return err
			}
...
func RegisterReferrerCmdV3() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			address, err := cli.ParseAddress(args[1], &flags)
			if err != nil {
				return err
			}
...
func SetOracleAddressCmdV3() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cobra.MinimumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			oracleAddress, err := cli.ParseAddress(args[0], &flags)
			if err != nil {
				return err
			}
			err = cli.CallContractWithFlags(
