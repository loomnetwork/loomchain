  ...
  node('linux') {
    ...
    try {
      ...
      if (currentBuild.currentResult == 'FAILURE' || thisBuild == 'FAILURE') {
        ...
        slackSend channel: '#blockchain-engineers', color: '#FF0000', message: "${env.JOB_NAME} (LINUX) - #${env.BUILD_NUMBER} Failure after ${currentBuild.durationString.replace(' and counting', '')} (<a href=\"${env.BUILD_URL}\">Open</a>)"
      }
  ...
  node('windows') {
    ...
    try {
      ...
      if (currentBuild.currentResult == 'FAILURE' || thisBuild == 'FAILURE') {
        ...
        slackSend channel: '#blockchain-engineers', color: '#FF0000', message: "${env.JOB_NAME} (WINDOWS) - #${env.BUILD_NUMBER} Failure after ${currentBuild.durationString.replace(' and counting', '')} (<a href=\"${env.BUILD_URL}\">Open</a>)"
      }
      else if (currentBuild.currentResult == 'SUCCESS') {
  ...
  node('osx') {
    ...
    try {
      ...
      if (currentBuild.currentResult == 'FAILURE' || thisBuild == 'FAILURE') {
        ...
        slackSend channel: '#blockchain-engineers', color: '#FF0000', message: "${env.JOB_NAME} (OSX) - #${env.BUILD_NUMBER} Failure after ${currentBuild.durationString.replace(' and counting', '')} (<a href=\"${env.BUILD_URL}\">Open</a>)"
      }
      else if (currentBuild.currentResult == 'SUCCESS') {
