...
func (w *PlasmaCoinWorker) sendCoinEventsToDAppChain() error {
	...
	err = w.sendPlasmaDepositEventsToDAppChain(depositeEvents, 5, 500*time.Millisecond)
	if err != nil {
		return fmt.Errorf("failed to send plasma deposit events to dappchain. Error: %v", err)
	}

	err = w.sendPlasmaStartedExitEventsToDAppChain(startedExitEvents, 5, 500*time.Millisecond)
	if err != nil {
		return fmt.Errorf("failed to send plasma start exit events to dappchain. Error: %v", err)
	}

	err = w.sendPlasmaWithdrewEventsToDAppChain(withdrewEvents, 5, 500*time.Millisecond)
	if err != nil {
