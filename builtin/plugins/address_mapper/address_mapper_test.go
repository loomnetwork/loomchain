...
func (s *AddressMapperTestSuite) TestBinanceSigRecovery() {
	...
	// sig generated by signing the hash
	sig, err := hex.DecodeString("04146ed25f73184258fe71179e2d8a6d60b608986e5437f165bb93722af0161a66585e32505d2fcf04c8dae856d7944ca13e04695e97ebf505fde74874ca3518db1c")
	r.NoError(err)
	sum := sha256.Sum256(hash)
	addr, err := evmcompat.BitcoinRecover(sum[:], sig[1:])
	r.NoError(err)
