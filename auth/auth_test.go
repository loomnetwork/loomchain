...
func TestSignatureTxMiddlewareMultipleTxSameBlock(t *testing.T) {
	...
	state.SetFeature(feature.IncrementNonceOnFailedTxFeature, true)
	...
	state2.SetFeature(feature.IncrementNonceOnFailedTxFeature, true)
	...
	state3.SetFeature(feature.IncrementNonceOnFailedTxFeature, true)
	...
	state3Dx.SetFeature(feature.IncrementNonceOnFailedTxFeature, true)
	...
	state4.SetFeature(feature.IncrementNonceOnFailedTxFeature, true)
	//If we get to tx with incrementing sequence numbers we should be fine in the same block
