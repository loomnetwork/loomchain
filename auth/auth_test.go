...
func TestSignatureTxMiddleware(t *testing.T) {
	...
	state := loomchain.NewStoreState(nil, store.NewMemStore(), nil, abci.Header{}, nil, nil)
...
func TestSignatureTxMiddlewareMultipleTxSameBlock(t *testing.T) {
	...
	state := loomchain.NewStoreState(ctx, store.NewMemStore(), nil, abci.Header{Height: 27}, nil, nil)
	...
	state2 := loomchain.NewStoreState(ctx2, store.NewMemStore(), nil, abci.Header{Height: 27}, nil, nil)
	...
	state3 := loomchain.NewStoreState(ctx3, store.NewMemStore(), nil, abci.Header{Height: 27}, nil, nil)
	...
	state3Dx := loomchain.NewStoreState(ctx3Dx, store.NewMemStore(), nil, abci.Header{Height: 27}, nil, nil)
	...
	state4 := loomchain.NewStoreState(ctx4, store.NewMemStore(), nil, abci.Header{Height: 28}, nil, nil)
