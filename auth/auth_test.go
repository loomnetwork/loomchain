...
func TestSignatureTxMiddlewareMultipleTxSameBlock(t *testing.T) {
	...
	state.SetFeature(loomchain.IncrementNonceOnFailedTxFeature, true)
	...
	state2.SetFeature(loomchain.IncrementNonceOnFailedTxFeature, true)
	...
	state3.SetFeature(loomchain.IncrementNonceOnFailedTxFeature, true)
	...
	state3Dx.SetFeature(loomchain.IncrementNonceOnFailedTxFeature, true)
	...
	state4.SetFeature(loomchain.IncrementNonceOnFailedTxFeature, true)
	//If we get to tx with incrementing sequence numbers we should be fine in the same block
