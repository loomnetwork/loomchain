...
func TestChainConfigMiddlewareSingleChain(t *testing.T) {
	...
	state := loomchain.NewStoreState(nil, store.NewMemStore(), nil, abci.Header{ChainID: "default"}, nil, nil)
...
func TestChainConfigMiddlewareMultipleChain(t *testing.T) {
	state := loomchain.NewStoreState(nil, store.NewMemStore(), nil, abci.Header{ChainID: defaultLoomChainId}, nil, nil)
