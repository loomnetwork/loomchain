...
func TestBlockIndexStore(t *testing.T) {
	...
	tesBlockIndexStore(t, memoryStore)
	...
	tesBlockIndexStore(t, levelDbStore)
}

func tesBlockIndexStore(t *testing.T, bs BlockIndexStore) {
	for _, hashHeight := range hashHeights {
		require.NoError(t, bs.PutHashHeight(hashHeight.hash, hashHeight.height))
	}
	for _, hashHeight := range hashHeights {
		height, err := bs.GetHeight(hashHeight.hash)
		require.NoError(t, err)
