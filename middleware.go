...
type TxMiddleware interface {
	ProcessTx(state appstate.State, txBytes []byte, next TxHandlerFunc, isCheckTx bool) (TxHandlerResult, error)
}

type TxMiddlewareFunc func(state appstate.State, txBytes []byte, next TxHandlerFunc, isCheckTx bool) (TxHandlerResult, error)

func (f TxMiddlewareFunc) ProcessTx(
	state appstate.State, txBytes []byte, next TxHandlerFunc, isCheckTx bool,
) (TxHandlerResult, error) {
	return f(state, txBytes, next, isCheckTx)
}

type PostCommitHandler func(state appstate.State, txBytes []byte, res TxHandlerResult, isCheckTx bool) error

type PostCommitMiddleware interface {
	ProcessTx(state appstate.State, txBytes []byte, res TxHandlerResult, next PostCommitHandler, isCheckTx bool) error
}

type PostCommitMiddlewareFunc func(
	state appstate.State, txBytes []byte, res TxHandlerResult, next PostCommitHandler, isCheckTx bool,
) error
...
func MiddlewareTxHandler(
	...
	postChain := func(state appstate.State, txBytes []byte, res TxHandlerResult, isCheckTx bool) error { return nil }
	for i := len(postMiddlewares) - 1; i >= 0; i-- {
		m := postMiddlewares[i]
		localNext := postChain
		postChain = func(state appstate.State, txBytes []byte, res TxHandlerResult, isCheckTx bool) error {
			return m.ProcessTx(state, txBytes, res, localNext, isCheckTx)
		}
	}

	next := TxHandlerFunc(func(state appstate.State, txBytes []byte, isCheckTx bool) (TxHandlerResult, error) {
	...
	for i := len(middlewares) - 1; i >= 0; i-- {
		...
		nextLocal := next
		next = func(state appstate.State, txBytes []byte, isCheckTx bool) (TxHandlerResult, error) {
...
var NoopTxHandler = TxHandlerFunc(func(state appstate.State, txBytes []byte, isCheckTx bool) (TxHandlerResult, error) {
