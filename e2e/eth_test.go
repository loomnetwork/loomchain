...
func TestEthJSONRPC2(t *testing.T) {
	tests := []struct {
		...
		{"blockNumber", "eth-1-test.toml", 4, 1, "empty-genesis.json", "eth-test-1-loom.yaml"},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			config, err := common.NewConfig(test.name, test.testFile, test.genFile, test.yamlFile, test.validators, test.accounts, 0, 1,false)
			if err != nil {
				t.Fatal(err)
			}
