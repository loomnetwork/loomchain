...
func (h *MigrationTxHandler) ProcessTx(
	...
	if !s.FeatureEnabled(features.MigrationTxFeature, false) {
	...
	migrationRun := s.Get(migrationKey(tx.ID))
	if migrationRun != nil {
		return r, fmt.Errorf("migration ID %d has already been processed", tx.ID)
	}

	id := fmt.Sprint(tx.ID)
	if !s.FeatureEnabled(features.MigrationFeaturePrefix+id, false) {
	...
	migrationCtx := migrations.NewMigrationContext(h.Manager, h.CreateRegistry, s, origin)
	if err := migrationFn(migrationCtx, tx.Input); err != nil {
		return r, errors.Wrapf(err, "migration %d failed", int32(tx.ID))
	}

	if s.FeatureEnabled(features.MigrationTxVersion1_1Feature, false) {
		s.Set(migrationKey(tx.ID), []byte{1})
	} else {
		s.Set(migrationKey(tx.ID), msg.Data)
	}
