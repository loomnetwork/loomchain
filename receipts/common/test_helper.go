...
func MockState(height uint64) loomchain.State {
	...
	return loomchain.NewStoreState(context.Background(), store.NewMemStore(), nil, header, nil, nil)
}
...
func MockStateTx(state loomchain.State, height, TxNum uint64) loomchain.State {
	...
	return loomchain.NewStoreState(context.Background(), state, nil, header, nil, nil)
}
