...
func (e Evm) Call(caller, addr loom.Address, contractVersion string, input []byte, value *loom.BigUInt) ([]byte, error) {
	var err error
	var usedGas uint64
	defer func(begin time.Time) {
		lvs := []string{"method", "DeliverTx", "error", fmt.Sprint(err != nil)}
...
func (e Evm) StaticCall(caller, addr loom.Address, contractVersion string, input []byte) ([]byte, error) {
	origin := common.BytesToAddress(caller.Local)
	contract := common.BytesToAddress(addr.Local)
	vmenv := e.NewEnv(origin)
