...
func (lvm LoomVm) saveEventsAndHashReceipt(addr loom.Address, events []*ptypes.EventData, err error) ([]byte, error) {
	state := *lvm.state.(*loomchain.StoreState)
	ssBlock := state.Block()
	var status int32
	if err == nil {
		status = 1
